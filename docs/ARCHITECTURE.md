...
2. `train_prob_model.py` が処理済み CSV から 3 つの dataset ルートを組み立てる。`legacy` は既存の tabular 特徴、`multihot` は draw ごとの multi-hot と番号レベルの `hit / frequency / gap` 特徴、`deepsets` は draw 内の当選番号集合に対して `number_norm / frequency / gap` を element feature として持つ。
...
4. `best static baseline`・`legacy`・`multihot`・`deepsets` 間の logloss 差に対して bootstrap CI と paired permutation test を計算し、calibration guardrail も含む `decision_summary` を生成する。
...
- `model_variants.py`: `legacy` / `multihot` / `deepsets` dataset 構築、shared scaler 用の reshape helper、recent input 再構築、manifest 由来の variant 解決。
...
- production artifact の既定経路は `legacy` のまま維持し、`multihot` / `deepsets` は `evaluation_model_variants` に含めて比較する。
...
- 採用判定は challenger ごとに `variant_vs_best_static` と `variant_vs_legacy` の統計条件を確認し、さらに選択 calibration 後の logloss / Brier / ECE guardrail を満たしたものだけ昇格候補にする。`deepsets_vs_multihot` は補助比較として保持する。
