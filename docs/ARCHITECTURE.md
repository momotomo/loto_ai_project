...
3. 同スクリプトが `data/eval_report_*.json`、`data/manifest_*.json`、`models/*_prob.keras`、`models/*_scaler.pkl`、`*_feature_cols.json` を生成し、data hash / seed / preset / Python / dependency versions も保存する。
4. `app.py` が成果物を読み、確率可視化・買い目生成・評価レポート・manifest を表示する。
5. `update_system.py` は 1 → 2 → 3 → `predict.py` の smoke test を順に実行する。開発 CI では `--skip_data_refresh` を使って network 依存を避けられる。
6. `scripts/run_experiment.py` は既存パイプラインを呼び出し、run ごとの config / source hash / artifact snapshot を `runs/` へ残す。
