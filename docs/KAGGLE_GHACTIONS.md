...
- `artifact_utils.py` / `calibration_utils.py` / `model_variants.py` / `evaluation_statistics.py` / `report_utils.py` も payload に含め、manifest / eval_report / calibration / multihot / deepsets 入力再構築 / 統計検定が Kaggle 側でも同じコードで生成されるようにする。
