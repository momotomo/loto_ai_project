...
def build_governance_report(
    ...
    lines.append("## Accepted Campaign Review")
    lines.append("")
    lines.append(
    ...
    # Build accepted-campaign summary inline
    _history = history or []
    _acs = build_accepted_campaign_summary(_history)
    n_acc = _acs.get("accepted_campaign_count", 0)
    n_counts = _acs.get("counts_toward_promotion_readiness_count", 0)
    consec_pos = _acs.get("consecutive_accepted_positive_signals", 0)
    acc_names = _acs.get("accepted_campaign_names") or []

    lines.append("| Metric | Value |")
    lines.append("|--------|-------|")
    lines.append(f"| Accepted campaigns | **{n_acc}** |")
    lines.append(f"| Campaigns counting toward promotion readiness | {n_counts} |")
    lines.append(f"| Consecutive accepted positive signals | {consec_pos} |")
    lines.append("")

    if acc_names:
        lines.append(
            f"Accepted campaigns: "
            + ", ".join(f"`{n}`" for n in acc_names)
        )
    else:
    ...
    lines.append("")
    # Build from the stability dict we already have
    _prr = build_promotion_review_readiness(
        _history,
        stability,
        promotion_gate=promotion_gate,
        regression_alert=regression_alert,
    )
    prr_ready = _prr.get("ready_for_promotion_review", False)
    prr_emoji = "✅" if prr_ready else "❌"
    prr_label = "READY FOR PROMOTION REVIEW" if prr_ready else "NOT YET READY"
    lines.append(f"**{prr_emoji} {prr_label}**")
    lines.append("")
    lines.append(_prr.get("rationale", ""))
    lines.append("")

    prr_blockers = _prr.get("blockers") or []
    if prr_blockers:
        lines.append("Readiness blockers:")
        for b in prr_blockers:
            lines.append(f"  - ❌ {b}")
        lines.append("")

    if not prr_ready:
        lines.append(
            "> ❌ **Why current accepted evidence is not enough**: "
            + (" / ".join(prr_blockers) if prr_blockers else "See blockers above.")
        )
    else:
        lines.append(
            "> ✅ **Accepted evidence is sufficient for promotion review.** "
            "Read the full review bundle before any production decision."
        )
    lines.append("")
...
def save_governance_artifacts(
    ...
    # 4. Governance report (includes comparability + acceptance + review sections)
    ...
    # 5. Accepted campaign summary (accepted-only view of history)
    acc_summary_paths = save_accepted_campaign_summary_artifacts(history, data_dir=data_dir)
    paths.update(acc_summary_paths)

    # 6. Promotion review readiness
    prr_paths = save_promotion_review_readiness_artifacts(
        history,
        stability,
        data_dir=data_dir,
        promotion_gate=gate,
    ...
    paths.update(prr_paths)

    # 7. Accepted campaign review bundle (combined review document)
    review_bundle_paths = save_accepted_campaign_review_bundle_artifacts(
        history,
        stability,
        data_dir=data_dir,
        promotion_gate=gate,
    ...
    paths.update(review_bundle_paths)
